        ...
        <NextLink href={step.href}>
          <a className="relative flex items-start group" aria-current="step">
            ...
            <span className="flex flex-col min-w-0 ml-4">
              ...
              <span className="text-sm text-blue-200">{step.description}</span>
            </span>
          </a>
        </NextLink>
      </>
      ...
      <>
        ...
        <NextLink href={step.href}>
          <a className="relative flex items-start group">
            <span className="flex items-center h-9" aria-hidden="true">
              <span className="relative z-10 flex items-center justify-center w-8 h-8 bg-blue-700 border-2 border-gray-300 rounded-full group-hover:border-gray-400">
          ...
          <a className="relative flex items-start group">
            ...
            <span className="flex flex-col min-w-0 ml-4">
              ...
              <span className="text-sm text-blue-200">
                {/* {step.description} */} asdkjaskdljasd
              </span>
            </span>
          </a>
        </NextLink>
    ...
    <nav className="mt-20" aria-label="Progress">
      <ol role="list" className="overflow-hidden">
        {list.map((step, stepIdx) => (
          ...
          >
            {step.status === 'complete' && renderCompletedStep(step)}
            {step.status === 'current' && renderCurrentStep(step)}
            {step.status === 'upcoming' && renderUpcomingStep(step)}
          </li>
        ))}
      </ol>
    </nav>
